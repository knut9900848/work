              ...
              <v-form v-model="valid">
                <v-container>
                  <v-row>
                    <v-col cols="12">
                      <v-textarea
                        v-model="saleOrderLine.description"
                        label="description"
                    ...
                    <!-- Item Name -->
                    <v-col cols="12" md="6">
                      <v-text-field v-model="saleOrderLine.name" label="Item Name" required></v-text-field>
                    </v-col>

                    <!-- Item Size -->
                    <v-col cols="12" md="6">
                      <v-text-field
                        v-model="saleOrderLine.size"
                        label="Size(Model)"
                        required
                    ...
                    <v-col cols="12" md="6">
                      <v-currency-field
                        v-model="saleOrderLine.quantity"
                        label="Quantity"
                        required
                        type="number"
      ...
      <v-card-text class="wrap-component-table">
        <v-simple-table class="ies-purchase-order-content-dialog-table">
          <template v-slot:default>
            ...
            <tbody>
              <tr v-for="(item, index) in saleOrderLines" :key="index">
            ...
            <tfoot>
              <tr>
                ...
                <td colspan="1" class="text-right">
                  <v-chip>{{ formatPrice(saleOrderTotalPrice) }}</v-chip>
                </td>
              </tr>
              <tr v-if="saleOrderOption.discount > 0">
                <td colspan="7" class="text-right">DISCOUNT</td>
                <td colspan="1" class="text-right">
                  <v-chip>{{ formatPrice(saleOrderOption.discount) }}</v-chip>
                </td>
              </tr>
              <tr v-if="saleOrderOption.shipping_fee > 0">
                <td colspan="7" class="text-right">SHIPPING / HANDLING</td>
                <td colspan="1" class="text-right">
                  <v-chip>{{ formatPrice(saleOrderOption.shipping_fee) }}</v-chip>
                </td>
              </tr>
              <tr v-if="saleOrderOption.is_use_tax">
                <td colspan="7" class="text-right">TAX</td>
                <td colspan="1" class="text-right">
                  <v-chip>{{ formatPrice(saleOrderTax) }}</v-chip>
                </td>
              </tr>
              <tr>
                <td colspan="7" class="text-right font-weight-black">GRAND TOTAL</td>
                <td colspan="1" class="text-right">
                  <v-chip color="primary">{{ formatPrice(saleOrderGrandTotal) }}</v-chip>
                </td>
              </tr>
            </tfoot>
          </template>
...
export default {
  ...
  computed: {
    isButtonDisabled() {
      if (
        // this.saleOrderStatus === "OPENED" &&
        // this.permissions.includes("edit-sale-order") &&
        this.saleOrderMode === "edit" ||
        this.saleOrderMode === "create"
      ) {
        return false;
      } else {
        return true;
    ...
    saleOrderTotalPrice() {
      return this.saleOrderLines.reduce((acc, item) => {
    ...
    saleOrderTax() {
      if (this.saleOrderOption.is_use_tax) {
        return parseFloat(
          (this.saleOrderTotalPrice +
            this.saleOrderOption.shipping_fee -
            this.saleOrderOption.discount) *
            0.1
        );
      } else {
        return 0;
    ...
    ...mapState("sale_order", ["saleOrderStatus"]),

    ...mapState("sale_order", ["saleOrderOption", "saleOrderMode"]),

    ...mapState("sale_order_line", ["saleOrderLines", "saleOrderLine"])
  },

  methods: {
    ...mapMutations("sale_order_line", [
      "ADD_SALE_ORDER_LINE",
      "EDIT_SALE_ORDER_LINE",
      "UPDATE_SALE_ORDER_LINE",
      "RESET_SALE_ORDER_LINE",
      "REMOVE_SALE_ORDER_LINE"
    ]),

    closeItemDialog() {
      this.itemDialog = false;
      this.$nextTick(() => {
        this.RESET_SALE_ORDER_LINE();
