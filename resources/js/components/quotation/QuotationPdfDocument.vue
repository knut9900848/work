          ...
          <tbody>
            ...
            <tr v-for="(pdfDocument, index) in pdfDocuments" :key="index" v-else>
              ...
              <td class="text-center">
                <v-btn
                  @click.prevent="downloadPdfFile(pdfDocument.id)"
                  color="red darken-1"
                  dark
                  text
                  icon
...
export default {
  ...
  computed: {
    ...
    isApproveButtonDisabled() {
      if (
        ...
        (this.exchangeRate * this.totalPrice < 500000 &&
          (this.emergency === 0 || this.emergency === false))
      ) {
        return false;
      } else {
        return true;
    ...
    ...mapState("quotation", ["quotationId"])
  },
  ...
  methods: {
    ...
    initialize() {
      Axios.get("/api/quotations/" + this.quotationId + "/pdf-documents").then(
        response => {
    ...
    generatePdfDocument() {
      Axios.post("/api/quotations/" + this.quotationId + "/pdf-documents")
        .then(response => {
          this.pdfDocuments.push(response.data.pdf);
    ...
    updatePdfDocument(item) {
      ...
      Axios.put("/api/quotations/" + this.quotationId + "/pdf-documents/")
        .then(response => {
          this.$store.dispatch("SHOW_GLOBAL_SNACKBAR", {
            text: response.data.message
          });
