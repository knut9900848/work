      ...
      <v-toolbar-title>
        <v-badge color="error" bordered :content="itemCount" offset-x="15" offset-y="20">
          <v-chip dark color="blue">
            <v-icon dark>mdi-weight-kilogram</v-icon>
          </v-chip>
        </v-badge>&nbsp;
        Unit
      </v-toolbar-title>
  ...
  methods: {
    ...
    save() {
      if (this.editedIndex > -1) {
        Axios.put("/api/admin/units/" + this.editedItem.id, this.editedItem)
          .then(response => {
        ...
        Axios.post("/api/admin/units", this.editedItem)
          .then(response => {
    ...
    deleteItem(item) {
      ...
      if (confirm("Do you really want to delete?") === true) {
        Axios.delete("/api/admin/units/" + item.id).then(response => {
