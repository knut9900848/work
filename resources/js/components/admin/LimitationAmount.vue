      ...
      <v-dialog v-model="dialog" max-width="500px">
        <v-card>
          <v-toolbar flat color="blue-grey darken-1">
            <v-toolbar-title>LIMITATION AMOUNT</v-toolbar-title>
            <v-spacer></v-spacer>
          ...
          <v-card-text>
            <div class="mt-5">
              ...
              ></v-text-field>
              <v-text-field
                v-model="editedItem.limitation_amount"
                label="Amount"
                type="number"
                outlined
                :error-messages="errors.limitation_amount"
              ></v-text-field>
        ...
        <template v-slot:default>
          ...
          <tbody>
            <tr v-for="item in data" :key="item.id">
              ...
              <td class="text-center">{{ item.limitation_amount }}</td>
  ...
  methods: {
    ...
    save() {
      if (this.editedIndex > -1) {
        Axios.put(
          "/api/admin/limitation-amounts/" + this.editedItem.id,
          this.editedItem
        )
          .then(response => {
        ...
        Axios.post("/api/admin/limitation-amounts", this.editedItem)
          .then(response => {
    ...
    deleteItem(item) {
      ...
      if (confirm("Do you really want to delete?") === true) {
        Axios.delete("/api/default-data/limitation-amounts/" + item.id).then(
          response => {
