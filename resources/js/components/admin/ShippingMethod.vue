      ...
      <v-toolbar-title>
        <v-badge color="error" bordered :content="itemCount" offset-x="15" offset-y="20">
          <v-chip dark color="blue">
            <v-icon dark>mdi-truck-fast</v-icon>
          </v-chip>
        </v-badge>&nbsp;
        Shipping Methods
      </v-toolbar-title>
  ...
  methods: {
    ...
    save() {
      if (this.editedIndex > -1) {
        Axios.put(
          "/api/admin/shipping-methods/" + this.editedItem.id,
          this.editedItem
        )
          .then(response => {
        ...
        Axios.post("/api/admin/shipping-methods", this.editedItem)
          .then(response => {
    ...
    deleteItem(item) {
      ...
      if (confirm("Do you really want to delete?") === true) {
        Axios.delete("/api/admin/shipping-methods/" + item.id).then(
          response => {
