      ...
      <v-toolbar-title>
        <v-badge color="error" bordered :content="itemCount" offset-x="15" offset-y="20">
          <v-chip dark color="blue">
            <v-icon dark>mdi-currency-krw</v-icon>
          </v-chip>
        </v-badge>&nbsp;
        Labor Wage
      </v-toolbar-title>
      ...
      <v-dialog v-model="dialog" max-width="500px">
        <v-card>
          <v-toolbar flat color="blue-grey darken-1">
            <v-toolbar-title>Labor Wage</v-toolbar-title>
            <v-spacer></v-spacer>
          ...
          <v-card-text>
            <div class="mt-5">
              ...
              <v-text-field
                ...
                label="Multiple"
                outlined
                :error-messages="errors.multiple"
                type="number"
              ></v-text-field>
              <v-dialog
                ref="refStartDialog"
                v-model="startTimePickerDialog"
                :return-value.sync="editedItem.from"
                persistent
                width="290px"
              >
                ...
                <v-time-picker v-if="startTimePickerDialog" v-model="editedItem.from" full-width>
                  <v-spacer></v-spacer>
                  <v-btn text color="primary" @click="startTimePickerDialog = false">Cancel</v-btn>
                  <v-btn text color="primary" @click="$refs.refStartDialog.save(editedItem.from)">OK</v-btn>
                </v-time-picker>
              </v-dialog>
              <v-dialog
                ref="refEndDialog"
                v-model="endTimePickerDialog"
                :return-value.sync="editedItem.to"
                persistent
                width="290px"
              >
                <template v-slot:activator="{ on }">
                ...
                <v-time-picker v-if="endTimePickerDialog" v-model="editedItem.to" full-width>
                  <v-spacer></v-spacer>
                  <v-btn text color="primary" @click="endTimePickerDialog = false">Cancel</v-btn>
                  <v-btn text color="primary" @click="$refs.refEndDialog.save(editedItem.to)">OK</v-btn>
                </v-time-picker>
              </v-dialog>
  ...
  methods: {
    ...
    save() {
      if (this.editedIndex > -1) {
        Axios.put(
          "/api/admin/labor-wages/" + this.editedItem.id,
          this.editedItem
        )
          .then(response => {
        ...
        Axios.post("/api/admin/labor-wages", this.editedItem)
          .then(response => {
    ...
    deleteItem(item) {
      ...
      if (confirm("Do you really want to delete?") === true) {
        Axios.delete("/api/admin/labor-wages/" + item.id).then(response => {
