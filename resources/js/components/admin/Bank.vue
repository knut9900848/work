        ...
        <v-card>
          ...
          <v-card-text>
            <div class="mt-5">
              ...
              ></v-text-field>
              <v-text-field v-model="editedItem.account_number"
                label="Bank Account Number" 
                outlined
                :error-messages="errors.account_number"
              ></v-text-field>
              <v-text-field v-model="editedItem.swift_code"
              ...
              <v-textarea v-model="editedItem.description"
                ...
                outlined
              ></v-textarea>
              <v-checkbox v-model="editedItem.is_active"
                label="Activate" 
              ></v-checkbox>
            </div>
          </v-card-text>
          ...
          <tbody>
            <tr v-for="item in data" :key="item.id">
              ...
              <td class="text-center">
                <v-chip color="primary" v-if="item.is_active === true || item.is_active === 1">Active</v-chip>
  ...
  methods: {
    ...
    save() {
      if (this.editedIndex > -1) {
        Axios.put('/api/admin/banks/' + this.editedItem.id, this.editedItem)
        .then(response => {
        ...
        Axios.post('/api/admin/banks', this.editedItem)
        .then(response => {
    ...
    deleteItem(item) {
      ...
      if ( confirm("Do you really want to delete?") === true ) {
        Axios.delete('/api/default-data/banks/' + item.id)
        .then(response => {
