  ...
  methods: {
    ...
    save() {
      if (this.editedIndex > -1) {
        Axios.put(
          "/api/admin/delivery-terms/" + this.editedItem.id,
          this.editedItem
        )
          .then(response => {
        ...
        Axios.post("/api/admin/delivery-terms", this.editedItem)
          .then(response => {
    ...
    deleteItem(item) {
      ...
      if (confirm("Do you really want to delete?") === true) {
        Axios.delete("/api/admin/delivery-terms/" + item.id).then(response => {
