      ...
      <v-dialog v-model="dialog" max-width="500px">
        <v-card>
          <v-toolbar flat color="blue-grey darken-1">
            <v-toolbar-title>Manpower Position</v-toolbar-title>
            <v-spacer></v-spacer>
          ...
          <v-card-text>
            <div class="mt-5">
              ...
              ></v-text-field>
              <v-select
                v-model="editedItem.manpower_discipline_id"
                :items="manpower_disciplines"
                item-text="name"
                item-value="id"
                label="Disciplines"
                outlined
                :error-messages="errors.manpower_discipline_id"
              ></v-select>
  ...
  methods: {
    ...
    save() {
      if (this.editedIndex > -1) {
        Axios.put(
          "/api/admin/manpower-positions/" + this.editedItem.id,
          this.editedItem
        )
          .then(response => {
        ...
        Axios.post("/api/admin/manpower-positions", this.editedItem)
          .then(response => {
    ...
    deleteItem(item) {
      ...
      if (confirm("Do you really want to delete?") === true) {
        Axios.delete("/api/admin/manpower-positions/" + item.id).then(
          response => {
    ...
    getManpowerDisciplines() {
      Axios.get("/api/admin/manpower-disciplines").then(response => {
        this.manpower_disciplines = response.data.data;
      });
    }
  }
};
