...
<head>
	...
	<style>
		@font-face {
			...
			src: {
					{
					storage_path('app/fonts/Roboto/Roboto-Regular.ttf')
				}
			}
		...
		<table cellpadding="0" cellspacing="0" class="attention-table">
			<tbody>
				...
				<tr>
					<th>Address</th>
					<td>
						@if(isset($object->company->street1)){{ $object->company->street1 }},&nbsp;@endif
						@if(isset($object->company->street2)){{ $object->company->street2 }}@endif
						@if(isset($object->company->state)){{ $object->company->state }}@endif
						@if(isset($object->company->city)){{ $object->company->city }}@endif
						@if(isset($object->company->country->name)){{ $object->company->country->name }}@endif
					</td>
				</tr>
				<tr>
					<th>Project</th>
